...
export function parseAnalysis(
  ...
  const combined = [summary, ...rationale, ...limitations, ...staffReviewChecklist]
    .join(" ");
  if (/\b(accept|reject|dispatch|deploy|create)\b.{0,35}\b(automatically|now)\b/i.test(combined)) {
    throw new Error("INVALID_MODEL_RESPONSE");
  }
  const identifiers = combined.match(/\b(?:B\d{3,}|route\s+\d+)\b/gi) ?? [];
  for (const identifier of identifiers) {
    const normalized = identifier.replace(/^route\s+/i, "");
    if (!context.allowedIdentifiers.some((allowed) => allowed.toLowerCase() === normalized.toLowerCase())) {
      throw new Error("INVALID_MODEL_RESPONSE");
    }
  }
...
export function buildGeminiPrompt(facts: DeterministicFacts): string {
  return [
...
function text(value: unknown): string {
  ...
  if (!trimmed || trimmed.length > 1200) throw new Error("INVALID_MODEL_RESPONSE");
  return trimmed;
}
